...
const normalizedLang = (giscusConfig.lang || siteConfig.lang || 'en').replace('_', '-');
const hasRequired = giscusConfig.repo && giscusConfig.repoId && giscusConfig.category && giscusConfig.categoryId;
...
onMounted(() => {
    const getTheme = () => (document.documentElement.classList.contains('dark') ? themeDark : themeLight);
