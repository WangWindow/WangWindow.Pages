...
onMounted(() => {
  ...
  const grouped: Record<number, Post[]> = filteredPosts.reduce(
    (acc, post) => {
      ...
      <div>
        <div class="flex flex-row w-full items-center h-15">
          ...
          <div class="w-[15%] md:w-[10%]">
            <div class="h-3 w-3 bg-none rounded-full outline-(--primary) mx-auto-outline-offset-2 z-50 outline-3">
            </div>
          </div>
        ...
        <div v-for="post in group.posts" :key="post.slug">
          <a :href="getPostUrlBySlug(post.slug)" :aria-label="post.data.title"
            ...
            <div class="flex flex-row justify-start items-center h-full">
              ...
              <!-- tag list -->
              <div
                class="hidden md:block md:w-[15%] text-left text-sm transition whitespace-nowrap text-ellipsis overflow-hidden text-30">
